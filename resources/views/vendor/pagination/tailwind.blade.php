@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-center">
        <!-- Mobile View -->
        <div class="flex justify-center gap-3 flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="relative inline-flex items-center px-4 py-2 text-xs font-semibold text-slate-300 bg-slate-50/50 border border-slate-150 rounded-xl cursor-default">
                    <i class="fa-solid fa-chevron-left mr-1.5 text-[10px]"></i> {!! __('pagination.previous') !!}
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-primary-50 hover:text-primary-600 hover:border-primary-200 active:scale-95 transition-all duration-300">
                    <i class="fa-solid fa-chevron-left mr-1.5 text-[10px]"></i> {!! __('pagination.previous') !!}
                </a>
            @endif

            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-primary-50 hover:text-primary-600 hover:border-primary-200 active:scale-95 transition-all duration-300">
                    {!! __('pagination.next') !!} <i class="fa-solid fa-chevron-right ml-1.5 text-[10px]"></i>
                </a>
            @else
                <span class="relative inline-flex items-center px-4 py-2 text-xs font-semibold text-slate-300 bg-slate-50/50 border border-slate-150 rounded-xl cursor-default">
                    {!! __('pagination.next') !!} <i class="fa-solid fa-chevron-right ml-1.5 text-[10px]"></i>
