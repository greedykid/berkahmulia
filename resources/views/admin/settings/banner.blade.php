...
@section('content')
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-500 mt-1">Atur gambar hero banner slider dan teks yang ditampilkan di halaman beranda.
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <form action="{{ route('admin.settings.updateHeroBanners') }}" method="POST" enctype="multipart/form-data"
            id="settings-form">
            @csrf
            <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm h-full">
                ...
                <div class="mb-5">
                    ...
                    @if(count($banners) > 0)
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            @foreach($banners as $index => $path)
                                <div
                                    class="relative group border border-slate-200 rounded-xl overflow-hidden bg-slate-50 p-1.5 hover:shadow-md transition-all duration-200">
                                    <div class="aspect-21/9 w-full rounded-lg overflow-hidden bg-slate-200 relative">
                                        <img src="{{ asset('storage/' . $path) }}" alt="Banner {{ $index + 1 }}"
                                            class="w-full h-full object-cover">
                                        <div
                                            class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                                            <span
                                                class="text-white text-[10px] font-semibold px-2 py-0.5 bg-slate-900/60 rounded-full">Banner
                                                {{ $index + 1 }}</span>
                                        </div>
                                    </div>
                                    <div class="p-2 flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <input type="checkbox" name="delete_banners[]" value="{{ $path }}"
                                                id="delete_banner_{{ $index }}"
                                                class="w-4 h-4 text-rose-600 border-slate-300 rounded focus:ring-rose-500 cursor-pointer">
                                            <label for="delete_banner_{{ $index }}"
                                                class="text-[11px] text-slate-600 font-medium cursor-pointer select-none">Hapus</label>
                                        </div>
                    ...
                    @else
                        <div
                            class="flex flex-col items-center justify-center p-6 border border-dashed border-slate-300 rounded-xl bg-slate-50/50">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Unggah Banner
                        Baru</label>
                    <div
                        class="border border-slate-200 rounded-xl p-3 bg-slate-50 hover:bg-slate-100 transition-all relative group cursor-pointer">
                        ...
                        <div class="flex flex-col items-center justify-center py-3 text-center">
                            <div
                                class="w-9 h-9 rounded-full bg-indigo-50 flex items-center justify-center text-sm mb-2 group-hover:scale-110 transition-transform duration-200">
                            ...
                            <span class="text-[10px] text-slate-400 mt-0.5">JPEG, PNG, JPG, WEBP (Max. 10MB) — Rasio
                                21:9</span>
                        </div>
                    </div>
                ...
                <div class="mt-5 flex justify-end">
                    <button type="submit"
                        class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-6 rounded-xl text-xs transition-all shadow-sm flex items-center gap-2 cursor-pointer">
        ...
        <div class="space-y-6">
            ...
            <form action="{{ route('admin.settings.updateAnnouncementText') }}" method="POST" id="announcement-text-form">
                @csrf
                <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h4 class="font-bold text-slate-800 text-sm uppercase tracking-wider">Pengumuman Top Bar</h4>
                            <p class="text-[11px] text-slate-500 mt-0.5">Ubah teks di bagian paling atas halaman situs.</p>
                        </div>
                    </div>

                    <div class="space-y-4">
                        <div>
                            <label for="store_announcement_text"
                                class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Teks
                                Pengumuman</label>
                            <input type="text" name="store_announcement_text" id="store_announcement_text"
                                value="{{ $announcementText }}" required
                                ...
                                placeholder="Selamat datang di Berkah Mulia!...">
                        </div>
                    </div>

                    <div class="mt-5 flex justify-end">
                        <button type="submit"
                            class="bg-primary-600 hover:bg-primary-700 text-white font-bold py-2.5 px-6 rounded-xl text-xs transition-all shadow-sm flex items-center gap-2 cursor-pointer">
                            <i class="fa-solid fa-floppy-disk"></i>
            ...
            <form action="{{ route('admin.settings.updateHeroText') }}" method="POST" id="hero-text-form">
                ...
                <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h4 class="font-bold text-slate-800 text-sm uppercase tracking-wider">Teks Banner</h4>
                            <p class="text-[11px] text-slate-500 mt-0.5">Ubah teks judul, badge, dan deskripsi di banner
                                beranda.</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="hero_badge"
                                class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Badge / Label
                                Kecil</label>
                            <input type="text" name="hero_badge" id="hero_badge" value="{{ $heroText['badge'] }}" required
                                class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-primary-400 transition-all"
                                placeholder="Contoh: Koleksi Baru 2026">
                        </div>
                        <div>
                            <label for="hero_title_line1"
                                class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Judul Baris
                                1</label>
                            <input type="text" name="hero_title_line1" id="hero_title_line1"
                                value="{{ $heroText['title_line1'] }}" required
                                class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-primary-400 transition-all"
                                placeholder="Contoh: Pakaian Lembut & Nyaman">
                        </div>
                        <div>
                            <label for="hero_title_line2"
                                class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Judul Baris 2
                                (Warna Aksen)</label>
                            <input type="text" name="hero_title_line2" id="hero_title_line2"
                                value="{{ $heroText['title_line2'] }}" required
                                class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-primary-400 transition-all"
                                placeholder="Contoh: Untuk Buah Hati Anda">
                        </div>
                        <div>
                            <label for="hero_description"
                                class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Deskripsi</label>
